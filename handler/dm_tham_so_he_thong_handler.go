...
type DmThamSoHeThongHandler struct {
	Repo repository.DmThamSoHeThongRepo
}

func (u *DmThamSoHeThongHandler) Insert(c echo.Context) error {
	data := new(DmThamSoHeThong)
...
func (u *DmThamSoHeThongHandler) Update(c echo.Context) (err error) {
...
func (u *DmThamSoHeThongHandler) Delete(c echo.Context) (err error) {
	id := c.Param("id")

	if len(id) == 0 {
		return helper.ResponseWithCode(c, http.StatusBadRequest, "Dữ liệu truyền vào không chính xác")
	}

	valConvert, err := strconv.ParseInt(id, 0, 64)
	if err != nil {
		return helper.ResponseWithCode(c, http.StatusBadRequest, "Dữ liệu không phải kiểu: int")
	}
...
func (u *DmThamSoHeThongHandler) GetById(c echo.Context) (err error) {
...
func (u *DmThamSoHeThongHandler) GetAll(c echo.Context) (err error) {
