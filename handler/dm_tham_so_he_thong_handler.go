...
func (u *DmThamSoHeThongHandler) Delete(c echo.Context) (err error) {
	...
	if len(id) == 0 {
		return  sdk.ResponseWithCode(c, http.StatusBadRequest, "Dữ liệu truyền vào không chính xác")
	}

	valConvert, err := strconv.ParseInt(id, 0, 64)
	if err != nil {
		return  sdk.ResponseWithCode(c, http.StatusBadRequest, "Dữ liệu không phải kiểu: int")
	}
