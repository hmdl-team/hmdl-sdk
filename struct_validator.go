...
import (
	...
	engTran "github.com/go-playground/validator/v10/translations/en"
...
func (cv *StructValidator) RegisterValidate() {
	if err := engTran.RegisterDefaultTranslations(cv.Validator, cv.Trans); err != nil {
		//log.Error(err.Error())
	}

	_ = cv.Validator.RegisterValidation("pwd", func(fl validator.FieldLevel) bool {
	...
	_ = cv.Validator.RegisterTranslation("pwd", cv.Trans, func(ut ut.Translator) error {
